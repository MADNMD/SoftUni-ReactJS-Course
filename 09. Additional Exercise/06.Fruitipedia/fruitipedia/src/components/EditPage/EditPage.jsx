...
export const EditPage = () => {
    ...
    useEffect(() => {
        fruitService.getOneFruit(fruitId)
            .then(fruitData => {
                formik.setValues({
                    name: fruitData.name || '',
                    imageUrl: fruitData.imageUrl || '',
                    description: fruitData.description || '',
                    nutrition: fruitData.nutrition || ''
                })
            })
    }, [fruitId]);

    const handleEditFruit = (fruitData) => {

        try {
            fruitService.editFruit(fruitId, fruitData);

            navigate(`/details-page/${fruitId}`);
        } catch (error) {
            console.log(error);
            throw error;
        }
